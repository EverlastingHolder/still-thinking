...
Подробная архитектура находится в [`LOGGING.md`](LOGGING.md).

Обязательные правила:

- все события проходят через проектный `LoggerClient`/logging infrastructure;
- feature, repository и service не создают `OSLog.Logger` напрямую;
- `print` и `debugPrint` не используются как постоянное логирование;
- logger scoped к стабильному каналу: `database`, `notifications`, `feature.reflection` и т. п.;
- активные каналы и минимальный уровень можно выбирать через Debug-конфигурацию;
- Xcode launch arguments и environment variables позволяют быстро включать только нужные каналы;
- debug-only интерфейс может переключать каналы во время работы приложения;
- Release не должен принимать debug-настройки, включающие подробный приватный вывод;
- отключённый канал не должен формировать дорогие сообщения и metadata;
- логирование тестируется через recording/no-op sink, а не через чтение Xcode Console.
...
- Одна ошибка не должна без причины логироваться повторно на каждом слое.
