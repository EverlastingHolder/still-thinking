...
- [x] Пользователь может создать мысль и выбрать дату.
- [x] Состояние переживает перезапуск приложения.
- [x] Preview и tests проходят.
...
- [x] База является источником истины.
- [x] Уведомление не содержит приватный текст по умолчанию.
- [x] Нет дублирующих pending requests.
...
- [x] Основной продуктовый цикл работает end-to-end.
- [x] Все переходы статуса протестированы.
- [x] Today не превращается в бесконтрольный inbox.
...
- [x] История мысли читаема и последовательна.
- [x] Archive покрывает основные статусы.
- [x] Поиск работает локально и приватно.
...
- [x] Приватный контент не раскрывается по умолчанию.
- [x] Блокировка не делает данные недоступными без понятного fallback.
- [x] Пользователь может удалить данные.
- [x] Каналы можно быстро переключать в Debug build.
